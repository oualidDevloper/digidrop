const axios = require('axios');

async function testFullPost() {
  const apiKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjE3Mzc2MzY5IiwiY3VzdG9tZXIiOiJGYWxzZSIsImp0aSI6IjNiN2M1OGVlLWVmZTQtNDNlMi1hNWQwLTUxMTVkODY2OWMzNCIsImV4cCI6MjA5MDM5Nzk4Nn0.Cztd92KE5hTe3WQipZ2f2QAo4OSX4mEf3XQF8xZjl2o";
  const shopId = "0fe55f4b-ab58-465a-a7a0-df8d5c452037";

  const payload = {
    name: "Full Integration Test",
    description: JSON.stringify({
        blocks: [{ key: "e", text: "Test", type: "unstyled", depth: 0, inlineStyleRanges: [], entityRanges: [], data: {} }],
        entityMap: {}
    }),
    status: "PUBLIC",
    visibility: "PUBLIC",
    unlisted: false,
    imageIds: [17444915], 
    categoryId: 3022951,
    variants: [
      {
        name: "Standard",
        price: { amount: 3.50, currency: "EUR" },
        stockLevel: 99999,
        minQuantity: 1,
        maxQuantity: 0,
        type: "SINGLE",
        chargeType: "ONE_TIME",
        deliveryType: "PRESET", 
        customerNote: "Thank you!",
        gateways: [{ id: 17375218 }] 
      }
    ]
  };

      const schemas = [
        { gateway: "Stripe" },
        { gateway: "STRIPE" },
        { gateway: "stripe" },
        { Gateway: "STRIPE" }
      ];

      for (const s of schemas) {
        payload.variants[0].gateways = [s];
        console.log("Testing:", JSON.stringify(s));
        try {
            const res = await axios.post(`https://business-api.antistock.io/v1/dash/shops/${shopId}/products`, payload, {
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" }
            });
            console.log("SUCCESS! Works with:", JSON.stringify(s));
            break;
        } catch (err) {
            console.error("FAILED with", s, err.response ? JSON.stringify(err.response.data) : err.message);
        }
      }
    }

testFullPost();
